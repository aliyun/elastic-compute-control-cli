...
func buildLocalizer(tag string, specs map[string]map[string]string) *Localizer {
	...
	bundle.AddMessages(language.English,
		...
		&i18n.Message{ID: "CleanupPrerequisiteNotFound", Other: "cleanup prerequisite was not found; the requested resource was not deleted"},
		&i18n.Message{ID: "CleanupTargetVerificationFailed", Other: "could not verify that the requested resource no longer exists"},
		...
		&i18n.Message{ID: "CredentialReauthenticationRequired", Other: "credential must be authenticated again"},
		&i18n.Message{ID: "CredentialAccountMismatch", Other: "credential account does not match the configured account"},
		&i18n.Message{ID: "CredentialIdentityChanged", Other: "credential identity changed during the operation"},
		&i18n.Message{ID: "CredentialStatePersistenceFailed", Other: "renewed credential state could not be persisted safely"},
		&i18n.Message{ID: "CredentialSourceDisabled", Other: "credential source is disabled"},
		&i18n.Message{ID: "UnsafeCredentialDebug", Other: "credential-bearing commands cannot run with Dara request debugging enabled"},
		...
		&i18n.Message{ID: "MissingCredentials", Other: "Alibaba Cloud credentials are required"},
		...
		&i18n.Message{ID: "OSSUtilAPIUnavailable", Other: "OSS call operation is unavailable"},
		&i18n.Message{ID: "OSSUtilUnavailable", Other: "OSS call runtime is unavailable"},
		&i18n.Message{ID: "OSSUtilVersionUnsupported", Other: "OSS call runtime version is unsupported"},
		&i18n.Message{ID: "OAuthLoginFailed", Other: "OAuth login failed"},
		&i18n.Message{ID: "OAuthLoginCanceled", Other: "OAuth login was canceled"},
		&i18n.Message{ID: "OAuthLoginDenied", Other: "OAuth authorization was denied"},
		&i18n.Message{ID: "OAuthLoginTimeout", Other: "OAuth login timed out"},
		&i18n.Message{ID: "OAuthServiceUnavailable", Other: "OAuth service is unavailable"},
		&i18n.Message{ID: "OAuthCallbackUnavailable", Other: "OAuth callback is unavailable"},
		&i18n.Message{ID: "OAuthAccountMismatch", Other: "OAuth account does not match"},
		&i18n.Message{ID: "OAuthAccountConfirmationRequired", Other: "OAuth account confirmation is required"},
		&i18n.Message{ID: "OAuthManualAuthorizationRequired", Other: "manual OAuth authorization is required"},
		&i18n.Message{ID: "OAuthPersistenceUncertain", Other: "OAuth state persistence is uncertain"},
		&i18n.Message{ID: "ProfileNotFound", Other: "profile is not configured"},
		&i18n.Message{ID: "UnsupportedCredentialMode", Other: "credential mode is not supported"},
		...
		&i18n.Message{ID: "SuggestionCredentialReauthenticationRequired", Other: "For native OAuth, rerun `ecctl configure --mode OAuth`; for Aliyun-compatible OAuth or CloudSSO, rerun `aliyun configure`, then retry."},
		&i18n.Message{ID: "SuggestionCredentialAccountMismatch", Other: "Verify the account in the selected CloudSSO profile or RAM role ARN, then authenticate the intended account again."},
		&i18n.Message{ID: "SuggestionCredentialIdentityChanged", Other: "Verify the selected credential source still returns the same account, user, or role, then start a new command."},
		&i18n.Message{ID: "SuggestionCredentialStatePersistenceFailed", Other: "Check that the private ecctl credential directory is writable only by the current user, then re-authenticate the profile."},
		&i18n.Message{ID: "SuggestionCredentialSourceDisabled", Other: "Unset `ALIBABA_CLOUD_DISABLE_EXTERNAL_PROCESS` only if this command is allowed to use External or CredentialsURI credentials."},
		&i18n.Message{ID: "SuggestionUnsafeCredentialDebug", Other: "Remove `dara` from the `DEBUG` environment variable, then retry."},
		...
		&i18n.Message{ID: "SuggestionOSSUtilAPIUnavailable", Other: "Install or upgrade aliyun CLI so that the bundled OSSUtil 2.x runtime supports this API."},
		&i18n.Message{ID: "SuggestionOSSUtilUnavailable", Other: "Install or upgrade aliyun CLI so that `aliyun ossutil version` reports a 2.x release."},
		&i18n.Message{ID: "SuggestionOSSUtilVersionUnsupported", Other: "Install or upgrade aliyun CLI so that `aliyun ossutil version` reports a 2.x release."},
		&i18n.Message{ID: "SuggestionOAuthLogin", Other: "Rerun the original ecctl configure command and complete authorization in the browser."},
		&i18n.Message{ID: "SuggestionOAuthCallback", Other: "Free one of the loopback ports 12345-12349, then rerun the original ecctl configure command."},
		&i18n.Message{ID: "SuggestionOAuthExpectedAccount", Other: "Rerun with `--expected-account-id <16-digit-account-id>` for the intended Alibaba Cloud account."},
		&i18n.Message{ID: "SuggestionOAuthManual", Other: "Rerun with `--manual` in a private terminal to print the one-time authorization URL."},
		&i18n.Message{ID: "SuggestionOAuthPersistenceUncertain", Other: "Rerun the same command; ecctl will reconcile the pending OAuth transaction before starting another login."},
		...
		&i18n.Message{ID: "CommandExample.ecctl.configure", Other: "  ecctl configure --mode OAuth --profile production\n  ecctl configure get\n  ecctl configure set region cn-hangzhou\n  ecctl configure use production"},
		...
		&i18n.Message{ID: "OAuthAuthorizationPrompt", Other: "Open this URL in a browser to authorize ecctl:"},
		&i18n.Message{ID: "OAuthAuthorizationSuccessPage", Other: "Authorization received. Return to the terminal to finish login."},
		&i18n.Message{ID: "OAuthAccountConfirmationPrompt", Other: "OAuth returned account {{.AccountID}} ({{.IdentityType}}). Type the complete account ID to confirm:"},
		...
		&i18n.Message{ID: "CommandShort.completion", Other: "Configure shell completion"},
		...
		&i18n.Message{ID: "CommandShort.ecctl.completion", Other: "Configure shell completion"},
		...
		&i18n.Message{ID: "CommandShort.ecctl.completion.zsh", Other: "Install zsh completion"},
		&i18n.Message{ID: "CommandLong.ecctl.completion", Other: "Configure shell completion.\n\nRun ecctl completion zsh to save the completion script and configure loading in .zshrc automatically. Uses ZDOTDIR when set, otherwise your home directory. Repeated installation updates the existing setup without appending duplicates. Open a new zsh session or run the printed source command to enable completion in your current session.\n\nThe bash, fish and powershell subcommands generate scripts; see their help for loading instructions."},
		&i18n.Message{ID: "CommandLong.ecctl.completion.zsh", Other: "Install zsh completion by saving the script to .zfunc/_ecctl and adding a managed loading block to .zshrc. Both paths are relative to ZDOTDIR when set, otherwise your home directory. Existing shell settings and permissions are preserved. Repeated installation updates the script and loading block without appending duplicates.\n\nThe current session still needs to load the script. Open a new zsh session or run the source command printed after installation. The script initializes zsh completion if needed.\n\nUse --no-descriptions to install completion without candidate descriptions."},
		&i18n.Message{ID: "CommandExample.ecctl.completion.zsh", Other: "  ecctl completion zsh\n  ecctl completion zsh --no-descriptions"},
		&i18n.Message{ID: "CompletionInstalled", Other: "Zsh completion installed."},
		&i18n.Message{ID: "CompletionAlreadyInstalled", Other: "Zsh completion is already installed; no changes were needed."},
		&i18n.Message{ID: "CompletionInstructions", Other: "Open a new zsh session to load completion automatically. To enable it in the current session, run: {{.Command}}"},
		&i18n.Message{ID: "CompletionInstallFailed", Other: "could not install zsh completion"},
		&i18n.Message{ID: "SuggestionCompletionInstall", Other: "Check the reported path and file permissions. If ZDOTDIR is set but empty, run unset ZDOTDIR to use your home directory. If .zshrc contains an incomplete ecctl completion block, repair its markers, then run ecctl completion zsh again."},
		...
		&i18n.Message{ID: "CommandShort.ecctl.update", Other: "Check for and install ecctl updates"},
		&i18n.Message{ID: "CommandExample.ecctl.update", Other: "  ecctl update --check\n  ecctl update\n  ecctl update 0.2.0\n  ecctl update --force"},
		&i18n.Message{ID: "UpdateFailureWithReason", Other: "ecctl update failed: {{.Reason}}"},
		&i18n.Message{ID: "UpdateFailed", Other: "ecctl update failed"},
		&i18n.Message{ID: "SuggestionUpdateFailed", Other: "Run `ecctl update --check` and verify network access and installation permissions."},
		&i18n.Message{ID: "UpdateUnavailable", Other: "the ecctl update source is temporarily unavailable"},
		&i18n.Message{ID: "UpdateIntegrityFailed", Other: "ecctl update integrity verification failed"},
		&i18n.Message{ID: "UpdateInvalidTarget", Other: "the requested ecctl update version is not valid for this installation"},
		&i18n.Message{ID: "UpdatePermissionDenied", Other: "ecctl does not have permission to update this installation"},
		&i18n.Message{ID: "UpdateBusy", Other: "another ecctl update is still in progress"},
		&i18n.Message{ID: "UpdateTimeout", Other: "the ecctl update timed out"},
		&i18n.Message{ID: "UpdateCanceled", Other: "the ecctl update was canceled"},
		&i18n.Message{ID: "UpdateInstallFailed", Other: "the ecctl update could not be installed"},
		&i18n.Message{ID: "SuggestionUpdateUnavailable", Other: "Retry later and verify network access to Alibaba Cloud OSS and GitHub Releases."},
		&i18n.Message{ID: "SuggestionUpdateIntegrityFailed", Other: "Do not bypass verification; retry later and report the diagnostic detail if it persists."},
		&i18n.Message{ID: "SuggestionUpdateInvalidTarget", Other: "Run `ecctl update --check`, use a valid SemVer, and pass `--force` only for an intentional reinstall or downgrade."},
		&i18n.Message{ID: "SuggestionUpdatePermissionDenied", Other: "Verify write permission for the installation, or repair the Homebrew installation and retry."},
		&i18n.Message{ID: "SuggestionUpdateBusy", Other: "Close other ecctl processes, wait for any pending update to finish, and retry."},
		&i18n.Message{ID: "SuggestionUpdateTimeout", Other: "Retry when the network is stable; the update made no unverified installation changes."},
		&i18n.Message{ID: "SuggestionUpdateCanceled", Other: "Run `ecctl update` again when you are ready to continue."},
		&i18n.Message{ID: "SuggestionUpdateInstallFailed", Other: "Review the diagnostic detail, correct the installation problem, and retry `ecctl update`."},
		&i18n.Message{ID: "UpdateAvailableNotice", Other: "ecctl {{.LatestVersion}} is available (current: {{.CurrentVersion}}). Run `ecctl update`."},
		&i18n.Message{ID: "FlagUsage.AlibabaCloudRegion", Other: "Alibaba Cloud region"},
		&i18n.Message{ID: "FlagUsage.EcctlConfigPath", Other: "ecctl configuration file path"},
		&i18n.Message{ID: "FlagUsage.AuthenticationMode", Other: "authentication mode (supported: OAuth)"},
		&i18n.Message{ID: "FlagUsage.ExpectedAccountID", Other: "expected 16-digit Alibaba Cloud account ID"},
		...
		&i18n.Message{ID: "FlagUsage.OAuthSiteType", Other: "OAuth site type (CN or INTL; default: CN)"},
		&i18n.Message{ID: "FlagUsage.OAuthManual", Other: "print the one-time OAuth URL instead of opening a browser"},
		...
		&i18n.Message{ID: "FlagUsage.CheckUpdateOnly", Other: "check for an update without installing it"},
		&i18n.Message{ID: "FlagUsage.ForceUpdate", Other: "reinstall the target version or allow a downgrade"},
		...
		&i18n.Message{ID: "UnsupportedOSSParameter", Other: "parameter is not supported for OSS calls"},
	...
	bundle.AddMessages(language.MustParse("zh-Hans"),
		...
		&i18n.Message{ID: "CleanupPrerequisiteNotFound", Other: "清理前置资源不存在，指定资源尚未删除"},
		&i18n.Message{ID: "CleanupTargetVerificationFailed", Other: "无法确认指定资源已不存在"},
		&i18n.Message{ID: "ConfigWriteFailed", Other: "配置写入失败"},
		&i18n.Message{ID: "ConflictingParameters", Other: "参数冲突"},
		&i18n.Message{ID: "CredentialReauthenticationRequired", Other: "凭证需要重新认证"},
		&i18n.Message{ID: "CredentialAccountMismatch", Other: "凭证账号与配置的账号不一致"},
		&i18n.Message{ID: "CredentialIdentityChanged", Other: "操作期间凭证身份发生变化"},
		&i18n.Message{ID: "CredentialStatePersistenceFailed", Other: "无法安全持久化续期后的凭证状态"},
		&i18n.Message{ID: "CredentialSourceDisabled", Other: "凭证来源已禁用"},
		&i18n.Message{ID: "UnsafeCredentialDebug", Other: "启用 Dara 请求调试时不能执行携带凭证的命令"},
		...
		&i18n.Message{ID: "MissingCredentials", Other: "必须提供阿里云凭证"},
		...
		&i18n.Message{ID: "OSSUtilAPIUnavailable", Other: "OSS 调用操作不可用"},
		&i18n.Message{ID: "OSSUtilUnavailable", Other: "OSS 调用运行环境不可用"},
		&i18n.Message{ID: "OSSUtilVersionUnsupported", Other: "OSS 调用运行环境版本不受支持"},
		&i18n.Message{ID: "OAuthLoginFailed", Other: "OAuth 登录失败"},
		&i18n.Message{ID: "OAuthLoginCanceled", Other: "OAuth 登录已取消"},
		&i18n.Message{ID: "OAuthLoginDenied", Other: "OAuth 授权被拒绝"},
		&i18n.Message{ID: "OAuthLoginTimeout", Other: "OAuth 登录超时"},
		&i18n.Message{ID: "OAuthServiceUnavailable", Other: "OAuth 服务暂时不可用"},
		&i18n.Message{ID: "OAuthCallbackUnavailable", Other: "OAuth 回调不可用"},
		&i18n.Message{ID: "OAuthAccountMismatch", Other: "OAuth 账号不匹配"},
		&i18n.Message{ID: "OAuthAccountConfirmationRequired", Other: "需要确认 OAuth 账号"},
		&i18n.Message{ID: "OAuthManualAuthorizationRequired", Other: "需要手工完成 OAuth 授权"},
		&i18n.Message{ID: "OAuthPersistenceUncertain", Other: "OAuth 状态持久化结果不确定"},
		&i18n.Message{ID: "ProfileNotFound", Other: "配置档案未配置"},
		&i18n.Message{ID: "UnsupportedCredentialMode", Other: "不支持该凭证模式"},
		...
		&i18n.Message{ID: "SuggestionCredentialReauthenticationRequired", Other: "原生 OAuth 请重新执行 `ecctl configure --mode OAuth`；Aliyun-compatible OAuth 或 CloudSSO 请重新执行 `aliyun configure`，然后重试。"},
		&i18n.Message{ID: "SuggestionCredentialAccountMismatch", Other: "确认所选 CloudSSO profile 或 RAM 角色 ARN 中的账号正确，然后重新认证目标账号。"},
		&i18n.Message{ID: "SuggestionCredentialIdentityChanged", Other: "确认所选凭证来源仍返回同一账号、用户或角色，然后重新执行命令。"},
		&i18n.Message{ID: "SuggestionCredentialStatePersistenceFailed", Other: "确认 ecctl 私有凭证目录仅当前用户可写，然后重新认证该 profile。"},
		&i18n.Message{ID: "SuggestionCredentialSourceDisabled", Other: "仅在允许该命令使用 External 或 CredentialsURI 凭证时，取消设置 `ALIBABA_CLOUD_DISABLE_EXTERNAL_PROCESS`。"},
		&i18n.Message{ID: "SuggestionUnsafeCredentialDebug", Other: "从 `DEBUG` 环境变量中移除 `dara`，然后重试。"},
		...
		&i18n.Message{ID: "SuggestionOSSUtilAPIUnavailable", Other: "安装或升级 aliyun CLI，确保其内置的 OSSUtil 2.x 运行环境支持此 API。"},
		&i18n.Message{ID: "SuggestionOSSUtilUnavailable", Other: "安装或升级 aliyun CLI，确保 `aliyun ossutil version` 返回 2.x 版本。"},
		&i18n.Message{ID: "SuggestionOSSUtilVersionUnsupported", Other: "安装或升级 aliyun CLI，确保 `aliyun ossutil version` 返回 2.x 版本。"},
		&i18n.Message{ID: "SuggestionOAuthLogin", Other: "重新执行原始 ecctl configure 命令，并在浏览器中完成授权。"},
		&i18n.Message{ID: "SuggestionOAuthCallback", Other: "释放本机 12345-12349 中的一个回调端口，然后重新执行原始 ecctl configure 命令。"},
		&i18n.Message{ID: "SuggestionOAuthExpectedAccount", Other: "使用目标阿里云账号的 `--expected-account-id <16位账号ID>` 重新执行命令。"},
		&i18n.Message{ID: "SuggestionOAuthManual", Other: "请在私有终端中使用 `--manual` 重新执行命令，以输出一次性授权地址。"},
		&i18n.Message{ID: "SuggestionOAuthPersistenceUncertain", Other: "请重新执行同一命令；ecctl 会先恢复未完成的 OAuth 事务，再开始新的登录。"},
		...
		&i18n.Message{ID: "OAuthAuthorizationPrompt", Other: "请在浏览器中打开以下地址，授权 ecctl："},
		&i18n.Message{ID: "OAuthAuthorizationSuccessPage", Other: "已收到授权，请返回终端等待登录完成。"},
		&i18n.Message{ID: "OAuthAccountConfirmationPrompt", Other: "OAuth 返回账号 {{.AccountID}}（{{.IdentityType}}）。请输入完整账号 ID 进行确认："},
		...
		&i18n.Message{ID: "CommandShort.completion", Other: "配置 shell 自动补全"},
		&i18n.Message{ID: "CommandShort.help", Other: "显示命令帮助"},
		&i18n.Message{ID: "CommandShort.ecctl", Other: "Agent 优先的弹性计算控制器"},
		&i18n.Message{ID: "CommandShort.ecctl.completion", Other: "配置 shell 自动补全"},
		...
		&i18n.Message{ID: "CommandShort.ecctl.completion.zsh", Other: "安装 zsh 自动补全"},
		&i18n.Message{ID: "CommandLong.ecctl.completion", Other: "配置 shell 自动补全。\n\n执行 ecctl completion zsh 会自动保存补全脚本并配置 .zshrc 加载。设置了 ZDOTDIR 时使用该目录，否则使用用户主目录。重复安装会更新已有配置，不会重复追加。安装后打开新的 zsh 终端，或执行结果中的 source 命令，在当前终端启用补全。\n\nbash、fish 和 powershell 子命令生成补全脚本，加载方式请查看各子命令帮助。"},
		&i18n.Message{ID: "CommandLong.ecctl.completion.zsh", Other: "安装 zsh 自动补全：将脚本保存到 .zfunc/_ecctl，并在 .zshrc 中添加受管理的加载配置段。设置了 ZDOTDIR 时，这两个路径位于该目录下，否则位于用户主目录下。保留已有 shell 设置和文件权限。重复执行会更新脚本和加载配置，不会重复追加。\n\n当前终端仍需加载脚本：打开新的 zsh 终端，或执行安装结果中的 source 命令。脚本会在需要时初始化 zsh 补全系统。\n\n使用 --no-descriptions 可安装不含候选项描述的补全脚本。"},
		&i18n.Message{ID: "CommandExample.ecctl.completion.zsh", Other: "  ecctl completion zsh\n  ecctl completion zsh --no-descriptions"},
		&i18n.Message{ID: "CompletionInstalled", Other: "已安装 zsh 自动补全。"},
		&i18n.Message{ID: "CompletionAlreadyInstalled", Other: "zsh 自动补全已安装，无需修改。"},
		&i18n.Message{ID: "CompletionInstructions", Other: "打开新的 zsh 终端会自动加载补全。要在当前终端启用，请执行：{{.Command}}"},
		&i18n.Message{ID: "CompletionInstallFailed", Other: "无法安装 zsh 自动补全"},
		&i18n.Message{ID: "SuggestionCompletionInstall", Other: "检查错误中的路径和文件权限；如果 ZDOTDIR 已设置但为空，请执行 unset ZDOTDIR 以使用用户主目录；如果 .zshrc 中的 ecctl 补全配置段不完整，请修复起止标记后重新执行 ecctl completion zsh。"},
		...
		&i18n.Message{ID: "CommandShort.ecctl.update", Other: "检查并安装 ecctl 更新"},
		&i18n.Message{ID: "CommandExample.ecctl.update", Other: "  ecctl update --check\n  ecctl update\n  ecctl update 0.2.0\n  ecctl update --force"},
		&i18n.Message{ID: "UpdateFailureWithReason", Other: "ecctl 更新失败：{{.Reason}}"},
		&i18n.Message{ID: "UpdateFailed", Other: "ecctl 更新失败"},
		&i18n.Message{ID: "SuggestionUpdateFailed", Other: "执行 `ecctl update --check`，并检查网络连接和安装目录权限。"},
		&i18n.Message{ID: "UpdateUnavailable", Other: "ecctl 更新源暂时不可用"},
		&i18n.Message{ID: "UpdateIntegrityFailed", Other: "ecctl 更新完整性校验失败"},
		&i18n.Message{ID: "UpdateInvalidTarget", Other: "请求的 ecctl 版本不适用于当前安装"},
		&i18n.Message{ID: "UpdatePermissionDenied", Other: "ecctl 没有更新当前安装所需的权限"},
		&i18n.Message{ID: "UpdateBusy", Other: "另一个 ecctl 更新仍在进行"},
		&i18n.Message{ID: "UpdateTimeout", Other: "ecctl 更新超时"},
		&i18n.Message{ID: "UpdateCanceled", Other: "ecctl 更新已取消"},
		&i18n.Message{ID: "UpdateInstallFailed", Other: "无法安装 ecctl 更新"},
		&i18n.Message{ID: "SuggestionUpdateUnavailable", Other: "请稍后重试，并检查到阿里云 OSS 和 GitHub Releases 的网络连接。"},
		&i18n.Message{ID: "SuggestionUpdateIntegrityFailed", Other: "不要绕过校验；请稍后重试，如持续失败请反馈 detail 中的诊断信息。"},
		&i18n.Message{ID: "SuggestionUpdateInvalidTarget", Other: "执行 `ecctl update --check`，使用有效的语义化版本；仅在明确需要重装或降级时使用 `--force`。"},
		&i18n.Message{ID: "SuggestionUpdatePermissionDenied", Other: "请检查安装目录写权限；如果由 Homebrew 安装，请修复 Homebrew 安装后重试。"},
		&i18n.Message{ID: "SuggestionUpdateBusy", Other: "请关闭其他 ecctl 进程，等待待处理更新结束后重试。"},
		&i18n.Message{ID: "SuggestionUpdateTimeout", Other: "请在网络稳定后重试；本次更新没有执行未经校验的安装变更。"},
		&i18n.Message{ID: "SuggestionUpdateCanceled", Other: "准备好继续时，请重新执行 `ecctl update`。"},
		&i18n.Message{ID: "SuggestionUpdateInstallFailed", Other: "请查看 detail 中的诊断信息，修复安装问题后重新执行 `ecctl update`。"},
		&i18n.Message{ID: "UpdateAvailableNotice", Other: "ecctl {{.LatestVersion}} 已发布（当前版本：{{.CurrentVersion}}）。执行 `ecctl update` 更新。"},
		&i18n.Message{ID: "FlagUsage.AlibabaCloudRegion", Other: "阿里云地域"},
		&i18n.Message{ID: "FlagUsage.EcctlConfigPath", Other: "ecctl 配置文件路径"},
		&i18n.Message{ID: "FlagUsage.AuthenticationMode", Other: "认证模式（支持：OAuth）"},
		&i18n.Message{ID: "FlagUsage.ExpectedAccountID", Other: "预期的 16 位阿里云账号 ID"},
		...
		&i18n.Message{ID: "FlagUsage.OAuthSiteType", Other: "OAuth 站点类型（CN 或 INTL；默认 CN）"},
		&i18n.Message{ID: "FlagUsage.OAuthManual", Other: "输出一次性 OAuth 地址，不自动打开浏览器"},
		...
		&i18n.Message{ID: "FlagUsage.CheckUpdateOnly", Other: "仅检查更新，不执行安装"},
		&i18n.Message{ID: "FlagUsage.ForceUpdate", Other: "重新安装目标版本，或允许降级"},
		...
		&i18n.Message{ID: "UnsupportedOSSParameter", Other: "OSS 调用不支持该参数"},
...
func (l *Localizer) ErrorSuggestion(code string) string {
	messageID := map[string]string{
		"CredentialAccountMismatch":          "SuggestionCredentialAccountMismatch",
		"CredentialReauthenticationRequired": "SuggestionCredentialReauthenticationRequired",
		"CredentialIdentityChanged":          "SuggestionCredentialIdentityChanged",
		"CredentialStatePersistenceFailed":   "SuggestionCredentialStatePersistenceFailed",
		"CredentialSourceDisabled":           "SuggestionCredentialSourceDisabled",
		"UnsafeCredentialDebug":              "SuggestionUnsafeCredentialDebug",
		...
		"OSSUtilAPIUnavailable":              "SuggestionOSSUtilAPIUnavailable",
		"OSSUtilUnavailable":                 "SuggestionOSSUtilUnavailable",
		"OSSUtilVersionUnsupported":          "SuggestionOSSUtilVersionUnsupported",
		"OAuthPersistenceUncertain":          "SuggestionOAuthPersistenceUncertain",
		...
		"UpdateFailed":                       "SuggestionUpdateFailed",
		"UpdateUnavailable":                  "SuggestionUpdateUnavailable",
		"UpdateIntegrityFailed":              "SuggestionUpdateIntegrityFailed",
		"UpdateInvalidTarget":                "SuggestionUpdateInvalidTarget",
		"UpdatePermissionDenied":             "SuggestionUpdatePermissionDenied",
		"UpdateBusy":                         "SuggestionUpdateBusy",
		"UpdateTimeout":                      "SuggestionUpdateTimeout",
		"UpdateCanceled":                     "SuggestionUpdateCanceled",
		"UpdateInstallFailed":                "SuggestionUpdateInstallFailed",
	}[code]
	if messageID == "" {
		return ""
	}
...
var flagUsageMessages = map[string]string{
	...
	"ecctl configuration file path":                                                     "FlagUsage.EcctlConfigPath",
	"authentication mode (supported: OAuth)":                                            "FlagUsage.AuthenticationMode",
	"expected 16-digit Alibaba Cloud account ID":                                        "FlagUsage.ExpectedAccountID",
	...
	"OAuth site type (CN or INTL; default: CN)":                                         "FlagUsage.OAuthSiteType",
	"print the one-time OAuth URL instead of opening a browser":                         "FlagUsage.OAuthManual",
	...
	"check for an update without installing it":                                         "FlagUsage.CheckUpdateOnly",
	"reinstall the target version or allow a downgrade":                                 "FlagUsage.ForceUpdate",
}
