...
设计目标：覆盖 ACK 控制面提供的报警规则启停与联系人组绑定能力。完整的报警规则定义、通知模板、阈值表达式及联系人/联系人组生命周期由 ARMS / 云监控 CMS 维护。ACK 只有联系人和联系人组删除接口，没有可验证的创建、查询生命周期，因此不将其建模为 `contact` 或 `contact-group` 资源。
...
- 指定 `--group-id` 时调用 [UpdateContactGroupForAlert](https://help.aliyun.com/zh/ack/ack-managed-and-ack-dedicated/developer-reference/api-cs-2015-12-15-updatecontactgroupforalert)：更新报警规则或规则集绑定的联系人组。
...
报警规则定义及联系人、联系人组的创建/查询/删除能力由阿里云监控产品（ARMS / CMS）承载。ACK 的孤立删除接口不形成资源生命周期，`ecctl ack alert` 不在此处兜底。
