...
.PHONY: help install build test coverage ci-test lint fmt clean generate drift drift-baseline drift-check prepare-public-release check-public-release check-release-version review-final review-e2e review-website
...
drift: ## Report OpenAPI metadata changes unmatched by resource bindings
	go run ./cmd/specdrift -spec-dir specs

drift-baseline: ## Record the current OpenAPI metadata as the drift baseline
	go run ./cmd/specdrift -spec-dir specs -write-baseline

drift-check: ## Fail when a binding misses an OpenAPI parameter added after the baseline
	go run ./cmd/specdrift -spec-dir specs -check
...
lint: ## Run formatting, vet, generated-code, and spec drift checks
