...
`ecctl` 支持阿里云 CLI 当前的同组十一种凭证模式，并把兼容的
`~/.aliyun/config.json` profile 作为只读 fallback。只有 `OAuth` 提供
`ecctl` 交互式配置流程：
...
其余模式通过 `aliyun configure --mode <Mode>`、环境变量，或直接向兼容配置
文件写入 profile 来配置。本地 AK 或 STS 凭证也可以直接设置：
...
[身份凭证](../credentials/index.md) 是这一领域的参考文档：模式选择、两个
配置文件及其能力差异、profile 与凭证解析顺序、身份固定、验证方式，以及
`DEBUG=dara` 的失败退出规则。每种凭证模式都有独立页面，可以从该总览或
侧边栏进入。
